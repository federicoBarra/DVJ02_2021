...
public class BezierCurve : MonoBehaviour {
    ...
    public bool GenerateParametrizationOnAwake = true;
	...
	void Awake(){
		dirty = true;
        if (GenerateParametrizationOnAwake)
	        GenerateBezierPoints(ParametrizationPrecision, MaxDistanceParametrization);
	}
    ...
    List<float> bezierTimeCuts;
    public int ParametrizationPrecision = 10000;
    public int MaxDistanceParametrization = 5;
    public List<float> GenerateBezierPoints(int precision, float maxDistanceBetWeenPoints)
    {
        bezierTimeCuts = new List<float>();
        float timeDist = 1f / precision;
        float t = 0;
        Vector3 lastPoint = GetPointAt(t);
        bezierTimeCuts.Add(t);
        while (t<1)
        {
            Vector3 q = GetPointAt(t+timeDist);

            if (Vector3.Distance(lastPoint, q) > maxDistanceBetWeenPoints)
            {
                bezierTimeCuts.Add(t);
                lastPoint = q;
            }
        ...
        bezierTimeCuts.Add(1);
        return bezierTimeCuts;
    }

    public List<float> GetParametrizedTimePoints(int precision, float maxDistanceBetWeenPoints)
    {
        if (precision == ParametrizationPrecision && maxDistanceBetWeenPoints == MaxDistanceParametrization && bezierTimeCuts!=null)
            return bezierTimeCuts;
        return GenerateBezierPoints(precision, maxDistanceBetWeenPoints);
    }
    public List<float> GetParametrizedTimePoints()
    {
        if (bezierTimeCuts != null)
            return bezierTimeCuts;
        return GenerateBezierPoints(ParametrizationPrecision, MaxDistanceParametrization);
    }

    public float GetCurveLength(int pression = 1000)
    {
        float length = 0;
        for (int i = 1; i < pression-1; i++)
        {
            length += Vector3.Distance( GetPointAt(1f/i), GetPointAt(1f/(i + 1)) );
        }
        return length;
    }
