...
namespace BezierSolution
{
	...
	public class BezierSpline : MonoBehaviour
	{
        ...
        /// <summary>
        /// TODO make something to specify that it flats Y axis
        /// </summary>
        ...
        /// <returns></returns>
        public Vector3 FindNearestPointToWithRange(Vector3 worldPos, out float normalizedT, float tRangeStart, float tRangeEnd, float step)
        {
            Vector3 result = Vector3.zero;
            normalizedT = -1f;
            ...
            float minDistance = Mathf.Infinity;
            for (float i = tRangeStart; i < tRangeEnd; i += step)
            {
                Vector3 thisPoint = GetPoint(i);
                Vector3 thisPointNoY = thisPoint;
                thisPointNoY.y = 0;
                float thisDistance = (worldPos - thisPointNoY).sqrMagnitude;
                if (thisDistance < minDistance)
                {
                    minDistance = thisDistance;
                    result = thisPoint;
            ...
            if (normalizedT < 0) // 
            {
                result = FindNearestPointTo(worldPos, out normalizedT, 1000);
                Debug.LogError("SOMETHING WENT AWFULLY WRONG");
            }
